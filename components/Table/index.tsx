...
export function useFilter<T extends Table.Props['data']>(
    data: T,
    { keywords, column }: Filter<T>,
    ...
    const compareStrings = (a: unknown, b: string) => {
        ...
        return a?.toString().toLocaleLowerCase().includes(
            b.toLocaleLowerCase(),
        );
    };
...
export function usePagination<T extends Table.Props['data']>(
    data: T,
    pageSize: number,
    initialPage = 0,
) {
    ...
    const maxPage = useMemo(
        () => Math.max(Math.ceil(data.length / pageSize) - 1, 0),
        [data.length],
    );
    const minmaxPage = useCallback(
        (page: number) => Math.min(Math.max(page, 0), maxPage),
        [maxPage],
    );
    ...
    const paginatedData = useMemo(
        () =>
            data.slice(
                page * pageSize,
                Math.min(page * pageSize + pageSize, data.length),
            ),
