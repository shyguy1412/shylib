...
export function bridge<T extends Record<string, unknown>>(
    module: T,
    namespace: string,
): void;
export function bridge<T extends Record<string, unknown>>(
    ...
    if (ipcMain) {
        ...
        if (typeof module != 'object') {
            throw new TypeError(
                "Expected 'module' to be an object, got " + typeof module,
            );
        }
        if (typeof namespace != 'string') {
            throw new TypeError(
                "Expected 'namespace' to be a string, got " + typeof namespace,
            );
        }
...
async function bridgeRender(namespace: string) {
    const methods: string[] = await ipcRenderer.invoke(
        `__module_bridge_get_api_methods_${namespace}`,
    );
    const constants: string[] = await ipcRenderer.invoke(
        `__module_bridge_get_constants_${namespace}`,
    );
    ...
    methods.reduce((prev, cur) => {
        prev[cur] = (...args: any[]) => {
            return ipcRenderer.invoke(
                `__module_bridge_${namespace}_${cur}`,
                ...args.map(serialize),
            );
        };
        return prev;
    ...
    await constants.reduce(async (prev, cur) => {
        (await prev)[cur] = await ipcRenderer.invoke(
            `__module_bridge_${namespace}_${cur}`,
        );
        return prev;
...
function bridgeMain<T extends Record<string, unknown>>(
    module: T,
    namespace: string,
) {
...
function deserialize(arg: any) {
    if (!arg || typeof arg != 'object' || !('__module_bridge_tag' in arg)) {
        return arg;
    }
    ...
    switch (arg['__module_bridge_tag']) {
        ...
        default:
            throw new Error('Invalid token tag ' + arg['__module_bridge_tag']);
    }
}
